...
export function SideBar({ role, links, setIsOptionsFormOpen, isOptionsFormOpen }: SideBarProps) {
    ...
    const handdleNavigate = (path: string) => navigate(path);
    ...
    const defaultLinks: LinkItem[] = [
        { key: "search", label: "Pesquisar", icon: <Search className="h-4 w-4 xxs:h-auto xxs:w-auto" />, onClick: () => handdleNavigate("/busca"), position: "top" },
    ...
    role === "ALUNO" && defaultLinks.push({
        key: "create",
        label: "Criar resumo",
        icon: <FilePlusIcon className="h-4 w-4 xxs:h-auto xxs:w-auto" />,
        onClick: () => setIsOptionsFormOpen?.(!(isOptionsFormOpen ?? false)),
        position: "top"
    });
    ...
    return (
        <header className="bg-white z-80 hover:z-90 fixed bottom-0 left-0 right-0 sm:top-0 sm:bottom-0 sm:right-auto sm:h-screen md:bg-transparent">
            <section
                ...
                `}
            >
                <nav className="bg-white w-full h-full flex sm:flex-col p-1 justify-between md:p-3">
                    <div className={`flex sm:flex-col gap-2 xxs:gap-6 md:gap-8 ${sideBar ? "pl-2" : "items-center"}`}>
                        ...
                        <ul className={`
                            ...
                            gap-4 text-xs flex sm:flex-col sm:text-sm
                        `}>
                            {topLinks.map((item) => (
                                <li
                                    key={item.key ?? item.label}
                                    onClick={() => { item.onClick?.(); if (item.to) handdleNavigate(item.to); }}
                                    className="flex gap-2 py-1 sm:hover:bg-[#DAE8FF] sm:hover:p-2 sm:hover:text-black sm:hover:scale-105 sm:hover:font-bold transform cursor-pointer sm:hover:shadow-lg sm:hover:w-full transition-all"
                                >
                                    <div className="cursor-pointer">{item.icon}</div>
                                    {sideBar && item.label && <h3>{item.label}</h3>}
                                </li>
                            ))}
                            ...
                            {role === "ADM" && (
                                <li
                                    ...
                                    className="flex gap-2 py-1 sm:hover:bg-[#DAE8FF] sm:hover:p-2 sm:hover:text-black sm:hover:scale-105 sm:hover:font-bold transform cursor-pointer sm:hover:shadow-lg sm:hover:w-full transition-all"
                                >
                                    <div className="cursor-pointer"><ToolCase /></div>
                                    {sideBar && <h3>Painel ADM</h3>}
                                </li>
                            )}
                        </ul>
                    </div>
                    ...
                    {role !== "ADM" && (
                        <section
                            className={`
                                gap-2 flex
                                ${sideBar ? "sm:flex sm:items-center" : "sm:flex-col"}
                                lg:pr-4
                                ${sideBar ? "items-start p-0 gap-4" : "items-center justify-center p-2"}
                                sm:hover:bg-[#DAE8FF] sm:hover:p-2 sm:hover:text-black sm:hover:scale-105 sm:hover:font-bold
                                transform transition-transform cursor-pointer sm:hover:shadow-lg sm:hover:w-full
                            `}
                        ...
                        >
                            ...
                            {sideBar && <h3>{bottomLinks[0]?.label ?? "User"}</h3>}
                        </section>
                    )}
