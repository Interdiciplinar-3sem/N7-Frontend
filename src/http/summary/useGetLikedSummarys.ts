import { useEffect } from "react"
import { useQuery } from "@tanstack/react-query"
import type { ResponseGetSummaryType } from "../types/responseGetSummary"
import { authFecth } from "../authFetch"
import { API_URL } from "../api"
import { useToast } from "../../contexto/toastContext"
import { getErrorMessage } from "../utils/getErrorMessage"

export const useGetLikedSummaries = () => {
    const { showError } = useToast()

    const query = useQuery({
        queryKey: ["get-liked-summaries"],
        queryFn: async (): Promise<ResponseGetSummaryType[]> => {
            const response = await authFecth(`${API_URL}/resumos/curtidos/me?page=0&size=20`)
            if (!response.ok) {
                throw new Error("Erro ao buscar resumos curtidos!")
            }

            const responseBody = await response.text()
            const result: ResponseGetSummaryType[] = responseBody.trim()
                ? JSON.parse(responseBody) : []

            return result.map((item) => ({
                studentId: item.studentId,
                summaryId: item.summaryId,
                titulo: item.titulo,
                conteudo: item.conteudo,
                reports: item.reports ?? 0,
                ativo: item.ativo,
                studentUrl: item.studentUrl ?? "/avatares/default.svg",
                studentNome: item.studentNome,
                totalCurtidas: item.totalCurtidas ?? 0,
                subjectId: item.subjectId,
                subjectNome: item.subjectNome,
                publico: item.publico,
                tags: item.tags
            }))
        },
        staleTime: 1000 * 60 * 5,
        retry: false
    })

    useEffect(() => {
        if (query.isError) {
            showError(getErrorMessage(query.error, "Erro ao carregar resumos curtidos"))
        }
    }, [query.error, query.isError, showError])

    return query
}